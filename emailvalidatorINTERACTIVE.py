...
# Function for removing unquoted and uncommented white space

def RemoveWhiteSpace(address):
    naturaladdress = ReplaceQuotesComments(address)     # A new variable so that an address with quotes/comments returns
    looping = True
    while looping:
        space = naturaladdress.find(' ')    # Finds the closest space
        while not UnBackslashed(naturaladdress, space):
            space = naturaladdress.find(' ', space)     # Finds a new one if it's backslashed
        if space == -1:
            looping = False     # Will end the loop if no space can be found
        else:
            address = address[:space] + address[space + 1:]
            naturaladdress = naturaladdress[:space] + naturaladdress[space + 1:]
    return address
...
def CommentContentsValid(address, openbracket, closebracket):
    ...
    for char in range(len(comment)):
        if comment[char] in restricted and UnBackslashed(address, char):
            return False
    return True
...
def QuotePositionValid(address, openquote, closequote):
    if closequote == len(address) or openquote == len(address):  # Needed to deal with potential indexing issues
        return False
    if \
            address[openquote - 1] != '.' \
            and address[closequote + 1] != '@' \
            and address[closequote + 1] != '.':
        return False
...
def QuoteContentsValid(address, openquote, closequote):
    ...
    for char in range(len(quote)):
        if quote[char] in restricted and UnBackslashed(address, char):
            return False
    return True
...
def ValidateQuotesComments(address):
    # address = RemoveWhiteSpace(address)     # White space is not counted for validating position fo quote/comment
...
def validIP(address):
    ...
    if ip[0:4] == 'IPv6':   # Detects if format is IPv6
        ipmode = 6
...
def DomainCharsCorrect(address):
                        ...
                        if DotsValid(email) == 'valid':
                            if LocalLengthValid(email):
                                if LocalCharsCorrect(email):
                                    ...
                                    else:   # These functions validate a non-IP domain:
                                        if DomainLengthValid(email):
                                            if DotInDomain(email):
                                                if LabelLengthValid(email):
                                                    if DomainCharsCorrect(email):
                                                        ...
                                                        result = 'Domain contained invalid characters'
                                                else:
...
print('Welcome to the email address validator Interactive mode')
